class Barco{
    ...
    display() {
      ...
      var index=floor(this.speed % this.animation.length); //redondear hacia abajo la velocidad

      push();
      translate(pos.x, pos.y);//para que tenga la animacion y se mueva
      imageMode(CENTER);
      image(this.animation[index], 0, this.boatPosition, this.width, this.height);//al ser una imagen lo primero en meter tiene que ser la imagen (this.barcos)
      pop();
    }
